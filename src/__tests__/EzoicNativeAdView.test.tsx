...
import { EzoicNativeAdView } from '../index';
...
describe('EzoicNativeAdView id coercion', () => {
  it('coerces a numeric id to a string', () => {
    const el = EzoicNativeAdView({ adUnitIdentifier: 12345 });
    expect(el.props.adUnitIdentifier).toBe('12345');
  });

  it('passes a string id through unchanged', () => {
    const el = EzoicNativeAdView({ adUnitIdentifier: '678' });
    expect(el.props.adUnitIdentifier).toBe('678');
  });
});

describe('EzoicNativeAdView click mapping', () => {
  it('maps the user-facing onClick onto the native onAdClick prop', () => {
    const onClick = jest.fn();
    const el = EzoicNativeAdView({ adUnitIdentifier: '1', onClick });
...
describe('EzoicNativeAdView event unwrapping', () => {
  it('unwraps nativeEvent for onError and forwards the payload', () => {
    const onError = jest.fn();
    const el = EzoicNativeAdView({ adUnitIdentifier: '1', onError });
...
describe('EzoicNativeAdView missing handlers', () => {
  it('leaves unset handlers undefined and does not throw', () => {
    const el = EzoicNativeAdView({ adUnitIdentifier: '1' });
  ...
  it('forwards style through the rest props', () => {
    ...
    const el = EzoicNativeAdView({ adUnitIdentifier: '1', style });
